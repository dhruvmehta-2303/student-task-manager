...
def add_student():
    ...
    if request.method == 'POST':
        ...
        mobile_number = request.form('mobile_number').strip().title()
        email = request.form('email').strip().title()
        course_name = request.form('course_name').strip().title()
...
def attendence_report():
    ...
    query = """
        ...
        ORDER BY attendence.attendence_id DESC
    """
